...
void	ft_draw_player(t_doom_editor *env)
{
	...
	data = &(env->data);
	player_pos = (t_point){data->camera.position.x / env->event.scale,
						data->camera.position.y / env->event.scale};
	...
	player_screen2.y = player_lookat.y - data->camera.raycast.plane.y / env->event.scale;
