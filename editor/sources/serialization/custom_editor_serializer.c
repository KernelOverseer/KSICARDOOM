...
/*   Updated: 2020/05/08 05:10:01 by abiri            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_sdl_image	*ft_get_texture_from_index(int index)
{
	if (index < 0 || index >= g_parsed_scene->textures_count)
		return (NULL);
	return (&g_parsed_scene->textures[index]);
}
...
int	ft_deserialize_wall(int fd, t_wall *wall)
{
	...
	if (texture_index < 0 || texture_index >= g_parsed_scene->textures_count)
		wall->texture = NULL;
	else
		wall->texture = &(g_parsed_scene->textures[texture_index]);
